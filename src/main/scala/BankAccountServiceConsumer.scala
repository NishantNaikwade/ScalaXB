...
object BankAccountServiceInvoker{
  ...
  def getAccountSummary : Future[Account] = {
    println("Making webservice call")
...
//    for{
//      response <- remote.service.getAccountSummary(Some("nnaiwkad@in.ibm.com"),Some("Nishant"),Some("Naikwade"),Some("9422926999"))
//    }yield{
//      accountStr = MyJsonProtocol.AccountSummaryResponseFormat.write(AccountSummaryResponse(response.accountId.get,response.accountType.get,account.get.balance.get)).toString()
//    }
